...
> *Resolved (07-08):* the headline is now labelled **"Total reachable"** (leads we haven't ruled out, incl. LinkedIn-only) with a separate honest **"Qualified"** line (only leads stamped `qualification_decision='qualified'`). Clients that qualify via other fields (paycaptain populates it barely; omnivate has no such column) show **n/a** on Qualified rather than a misleading near-zero. The deeper Cylindo schema revalidation (leads with no decision recorded) remains Omar's to own.
